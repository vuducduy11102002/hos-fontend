...
import { AppointmentRequestService, Doctor, DoctorService, RequestAppointment } from '@hospital/libs/services';
...
export class BookpointmentComponent implements OnInit {
    ...
    onSubmit() {
        ...
        this.appointmentRequestService.createRequestAppointment(requestpointment).subscribe(
            (response) => {
                ...
                if (error.status === 400 && error.error && error.error.errorType) {
                    ...
                    if (errorType === 'appointmentConflict') {
                        ...
                        this.messageService.add({
                            severity: 'error',
                            summary: 'Error',
                            detail: 'You have already submitted an appointment , please wait for approval'
                        });
                    }
                } else {
                    this.messageService.add({
                        severity: 'error',
                        summary: 'Error',
                        detail: 'An error occurred while submitting the appointment request.'
                    });
                }
            }
        );
